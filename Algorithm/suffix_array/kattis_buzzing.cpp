...
int main(){
    ...
    while(getline(cin,s) && s!=""){
        ...
        int n = ns.size();
        sa = new SuffixArray(ns, false);    
        seg = new segtree<ll, op, e>(sa->lcp);
        ll ans[n+1];
        //cout << n << "\n";
        ...
        for(int i = 2; i <= n; i ++){
            ...
            for(int j = 0; j < n - i +1 ; j++){
                //cout << sa->lcp.size() << " " << j << " " << i+j-1 << "\n";
                maxv = max(maxv,seg->prod(j+1,j+i-1+1));
            }
            ans[maxv] = i;
        }
        ll maxv = ans[n];
