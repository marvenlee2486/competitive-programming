...
void solve(){
    ...
    rep(i,0,n){
        int c=0;
        cin >> c;
        if(type[i]) frost.push_back(c);
        else fire.push_back(c);
    }
    sort(ALL(frost));
    sort(ALL(fire));
    reverse(ALL(frost));
    reverse(ALL(fire));
    int dou = min(fire.size(),frost.size());
    ll ans=0;
    rep(i,0,dou){
        ans+= frost[i]*2;
        ans += fire[i]*2;
    }
    if(frost.size()==dou && fire.size() == dou){
        ans -= min(frost[dou-1],fire[dou-1]);
    }
    rep(i,dou,frost.size()) ans+= frost[i];
    rep(i,dou,fire.size()) ans+= fire[i];
    cout << ans << "\n";
